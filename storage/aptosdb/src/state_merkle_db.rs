...
impl TreeWriter<StateKey> for StateMerkleDb {
    fn write_node_batch(&self, node_batch: &NodeBatch) -> Result<()> {
        ...
        let top_level_batches = SchemaBatch::new();
        let mut jmt_shard_batches: Vec<SchemaBatch> = Vec::with_capacity(NUM_STATE_SHARDS);
        for _ in 0..NUM_STATE_SHARDS {
            jmt_shard_batches.push(SchemaBatch::new());
        }
        node_batch.iter().try_for_each(|(node_key, node)| {
            if let Some(shard_id) = node_key.get_shard_id() {
                ...
                top_level_batches.put::<JellyfishMerkleNodeSchema>(node_key, node)
            }
        })?;

        TREE_COMMIT_POOL.scope(|s| {
            let mut state_merkle_batch = jmt_shard_batches.into_iter();
            for shard_id in 0..NUM_STATE_SHARDS {
        ...
        self.state_merkle_metadata_db
            .write_schemas(top_level_batches)
    }
}
