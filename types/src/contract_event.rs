...
impl ContractEvent {
    ...
    pub fn new_v1(struct_tag: StructTag, event_data: Vec<u8>) -> Self {
        ContractEvent::V1(ContractEventV1::new(struct_tag, event_data))
    }
}
...
impl ContractEventV1 {
    pub fn new(struct_tag: StructTag, event_data: Vec<u8>) -> Self {
        Self {
            struct_tag,
            event_data,
        }
...
impl std::fmt::Display for ContractEvent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let ContractEvent::V0(_) = self {
