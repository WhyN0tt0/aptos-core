...
impl Loader {
    ...
    fn type_to_type_tag_impl(&self, ty: &Type) -> PartialVMResult<TypeTag> {
        Ok(match ty {
            ...
            Type::Struct { name, .. } => {
                TypeTag::Struct(Box::new(self.struct_name_to_type_tag(name, &[])?))
            },
            Type::StructInstantiation { name, ty_args, .. } => {
                TypeTag::Struct(Box::new(self.struct_name_to_type_tag(name, ty_args)?))
            },
    ...
    fn struct_name_to_fully_annotated_layout(
        ...
        let count_before = *count;
        let struct_tag = self.struct_name_to_type_tag(name, ty_args)?;
